...
# Hyperparameters: train.py --evolve --epochs 2 --img-size 320, Metrics:
...
def train(
    ...
    resume: bool
        Whether use the pretrained model.
    epochs: int
    batch_size: int
    accumulate: int
        gradient accumulate can solve small batchsize problem.
    ...
    if resume:  # Load previously saved model
        if transfer:  # Transfer learning
            ...
            model.load_state_dict({k: v for k, v in chkpt['model'].items(
            ) if v.numel() > 1 and v.shape[0] != 255}, strict=False)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        print(('\n%8s%12s' + '%10s' * 7) % ('Epoch', 'Batch', 'xy',
                                            'wh', 'conf', 'cls', 'total', 'targets', 'time'))
        # Update scheduler
        scheduler.step()
        ...
        if not (opt.notest or (opt.nosave and epoch < 10)
            ...
            with torch.no_grad():
                results, maps = test.test(
                    cfg, data_cfg, batch_size=batch_size, img_size=img_size_test, model=model, conf_thres=0.1)
        # Write epoch results
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help='number of batches to accumulate before optimizing')
    parser.add_argument(
        '--cfg',
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        help='distributed training init method')
    parser.add_argument(
        '--rank',
        default=0,
        type=int,
    ...
    if opt.evolve:
        ...
        for _ in range(gen):
            ...
            # xy, wh, cls, conf, iou_t, lr0, lrf, momentum, weight_decay
